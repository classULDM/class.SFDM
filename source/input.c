  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_scf:
      ...
      if (ba.scf_tuning_index == 0){
        ...
        dxdy[index_guess] = 1./ba.Omega0_scf; //Corrected to run fractions of SFDM
      }
      break;
    case omega_ini_dcdm:
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  class_call(parser_read_double(pfc,"Omega_scf",&param3,&flag3,errmsg),
             errmsg,
             errmsg);
  //class_call(parser_read_double(pfc,"omega_scf",&param4,&flag4,errmsg),
  //           errmsg,
  //           errmsg);
  /* Test */
  ...
  class_test(((flag3 == _TRUE_)  && (flag4 == _TRUE_)) && ((param3 >= 0.) && (param3 >= 0.)),
             errmsg,
             "'Omega_scf' and 'omega_scf' can't be set positive simultaneusly.");
*/
  /* Complete set of parameters
  ...
  //  pba->Omega0_scf = param4 /pba->h/pba->h;
  //  Omega_tot += pba->Omega0_scf;
  //}
  /* Step 2 */
  if (flag1 == _FALSE_) {
  ...
  if (pba->Omega0_scf != 0.){
    ...
    class_call(parser_read_double(pfc,"ln10^{10}m_a",&param2,&flag2,errmsg),
                ...
                errmsg);                
    if (flag2 == _TRUE_){
      pba->scf_parameters[0] = pow(10,param2);
      }

    //class_read_double("theta_Ak",pba->theta_Ak);
    class_call(parser_read_double(pfc,"theta_Ak",&param2,&flag2,errmsg),
                errmsg,
                errmsg);
    if (flag2 == _TRUE_){
      pba->theta_Ak = param2;
    ...
    if (flag1 == _TRUE_){
      ...
      double Omega_ini_0 = 0, Omega_ini_1 = 0, H_ini = 0, m_in_Mpc=0, m_in_Mpc_a2_ini=0;
       ...
       m_in_Mpc = pba->scf_parameters[0]*1.56373846613383*1.e29;
       m_in_Mpc_a2_ini = pba->scf_parameters[0]*15.64;

      theta_ini = 0.4*15.64*pba->scf_parameters[0]/(pow(pba->Omega0_g+pba->Omega0_ur,0.5)*pba->H0);//2*m_in_Mpc_a2_ini/(pow(pba->Omega0_g+pba->Omega0_ur,0.5)*pba->H0); 
      ...
      //Omega_ini_1 = metric_shear_ini*a2_ini/(pow(pba->Omega0_g+pba->Omega0_ur,0.5)*pba->H0) * log(a_ini);
       //Omega_ini = pba->scf_parameters[pba->scf_tuning_index]+log(exp(Omega_ini_0)+exp(Omega_ini_1));
                // - Set up initial conditions
          pba->theta_phi_ini_scf = theta_ini;
          pba->Omega_phi_ini_scf = Omega_ini; //+ Omega_ini_1;//Omega_ini;
...
int input_default_params(struct background *pba,
  ...
  pba->Omega0_fld = 0.;
  pba->Omega0_scf = 0.;
  pba->Omega0_lambda = 1.-pba->Omega0_scf-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr - pba->Omega0_idr -pba->Omega0_idm;
  /** 8.a) Omega fluid */
  /** 8.a.1) PPF approximation */
  pba->use_ppf = _TRUE_;
